...
// Cloud/Production: Use HTTP (SSL handled by load balancer)
if (process.env.NODE_ENV === 'production') {
    server = http.createServer(app);
} else {
    // Local: Try to use HTTPS if certs exist
    try {
        const options = {
...
// Queue Abstraction - Redis Only
const queueOps = {
    push: async (lang, socketId) => {
        await dbClient.lPush(getQueueKey(lang), socketId);
    },
    pop: async (lang) => {
        return await dbClient.rPop(getQueueKey(lang));
    },
    returnToFront: async (lang, socketId) => {
        await dbClient.rPush(getQueueKey(lang), socketId);
    },
    remove: async (lang, socketId) => {
        await dbClient.lRem(getQueueKey(lang), 0, socketId);
    }
};
